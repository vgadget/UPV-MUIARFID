...
const { scene, camera, renderer } = setupScene(perspective = true);
...
function generarAntebrazoRobot(material) {
    ...
    let disco = new THREE.Mesh(
        new THREE.CylinderGeometry(radio_disco, radio_disco, 6, 35),
        material
    );
    obj.add(disco);
    ...
    let mano = new THREE.Mesh(
        new THREE.CylinderGeometry(radio_mano, radio_mano, 40, 35),
        material
    );
...
function generarBrazoRobot(material) {
    ...
    let baseBrazo = new THREE.Mesh(
        new THREE.CylinderGeometry(radio, radio, 18, 35),
        material
    );
